...
namespace com.softwarekey.Client.Sample
{
    ...
    public partial class MainForm : Form
    {
        ...
        private SampleSelfSignedLicense m_EvaluationLicense = null;
        private SampleLicense m_CurrentLicense = null;
        ...
        /// <summary>Gets whether or not the application's license is an evaluation.</summary>
        public bool IsEvaluation
        {
            get { return m_IsEvaluation; }
        }

        /// <summary>Gets whether or not the application's license is valid.</summary>
        ...
        {
            this.Text = "Simple Text Editor Self Signed Trial " + strFileName;
            ...
            {
                MessageBox.Show(this, "Error :" + ex, "Simple Text Editor Self Signed Trial", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            finally
            {
        ...
        private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                switch (MessageBox.Show(this, "The document is not saved. Do you wish to save now ?", "Simple Text Editor Self Signed Trial", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Exclamation))
                {
                ...
                switch (MessageBox.Show(this, "The document is not saved. Do you wish to save now ?", "Simple Text Editor Self Signed Trial", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Exclamation))
                {
                    case DialogResult.Yes:
                        if (!SaveFile())
                            return;
                        EditorTextBox.Text = "";
                        this.Text = "Simple Text Editor Self Signed Trial ";
                        break;
                    case DialogResult.No:
                        EditorTextBox.Text = "";
                        this.Text = "Simple Text Editor Self Signed Trial ";
        ...
        private void mnuOpen_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "File not found or permission denied", "Simple Text Editor Self Signed Trial", MessageBoxButtons.OK);
                return;
            }
            catch (Exception ex)
            {
                MessageBox.Show(this, "Error : " + ex.ToString(), "Simple Text Editor Self Signed Trial", MessageBoxButtons.OK);
                return;
            }
            finally
            {
        ...
        public void UpdateLicenseStatusProperty()
        {
            ...
            switch (m_CurrentLicense.LastError.ErrorNumber)
            {
                case LicenseError.ERROR_COULD_NOT_LOAD_LICENSE:
                    status.Append(m_CurrentLicense.LastError.ErrorNumber);
                    status.Append(": ");
                ...
                case LicenseError.ERROR_LICENSE_NOT_EFFECTIVE_YET:
                    ...
                    DateTime local = m_CurrentLicense.EffectiveStartDate.ToLocalTime();
                ...
                case LicenseError.ERROR_WEBSERVICE_RETURNED_FAILURE:
                    ...
                    status.Append(m_CurrentLicense.LastError.ExtendedErrorNumber);
                    status.Append(": ");
                    status.Append(LicenseError.GetWebServiceErrorMessage(m_CurrentLicense.LastError.ExtendedErrorNumber));
                    break;
                default:
                    //Show a standard error message.
                    status.Append(m_CurrentLicense.LastError.ErrorNumber);
                    status.Append(": ");
                    status.Append(m_CurrentLicense.LastError.ToString());
                    break;
            }
        ...
        public bool RefreshLicenseStatus()
        {
            ...
            {
                mnuRefreshLicense.Enabled = false;
                TimeSpan ts = m_CurrentLicense.EffectiveEndDate.ToLocalTime() - DateTime.Now.Date;
